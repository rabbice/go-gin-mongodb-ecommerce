...
func (app *CartHandler) AddToCart() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.IndentedJSON(200, gin.H{"message": "Item added to cart"})
	}
}

func (app *CartHandler) RemoveFromCart() gin.HandlerFunc {
	return func(c *gin.Context) {
		productQueryID := c.Query("id")
		if productQueryID == "" {
			log.Println("product id is empty")
...
func (app *CartHandler) BuyItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := database.MakeOrder(ctx, app.userCollection, userQueryID)
		if err != nil {
			c.IndentedJSON(http.StatusInternalServerError, err)
		}
...
func (app *CartHandler) InstantBuy() gin.HandlerFunc {
	return func(c *gin.Context) {
		productQueryID := c.Query("id")
		if productQueryID == "" {
			log.Println("product id is empty")
